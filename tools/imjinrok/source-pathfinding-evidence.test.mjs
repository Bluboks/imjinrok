...
test("extracts the hash-bound bounded pathfinding evidence fixture", () => {
  ...
  assert.deepEqual(report.candidateStateMachine.vector.map(({ state, nextState }) => [state, nextState]), [[2, 4], [4, 8], [8, 1], [1, 3], [3, 6], [6, 12], [12, 9], [9, 2]]);
});
...
test("reproduces wrapper capacity boundaries, exact candidate order, and strict first-entry ties", () => {
  ...
  assert.deepEqual(sourceCandidateVector({ x: 10, y: 10 }).map(({ coordinate }) => coordinate), [
...
test("reproduces goal, strict closest fallback, footprint block/OOB, frontier capacity, and wrapper failure vectors", () => {
  ...
  assert.deepEqual(replaySourcePathfinding({ start: { x: 2, y: 2 }, goal: { x: 3, y: 3 }, entity, terrain: open, width: 8, height: 8, acceptedNodeCounter: 6001 }), { returnValue: -1, reason: "accepted-node-limit" });
});
...
test("fails closed for malformed pure inputs and hash-bound source tampering", (t) => {
  ...
  assert.throws(() => sourceCandidateVector({ x: 1.5, y: 0 }), /integer x\/y/u);
