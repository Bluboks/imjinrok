...
function callClosure(references, functions, root) { const functionsByEntry = new Set(functions.map((entry) => entry.entry)); const visited = new Set([root]); const pending = [root]; for (let index = 0; index < pending.length; index += 1) for (const reference of references) if (reference.type.includes("CALL") && reference.fromFunctionEntry === pending[index] && functionsByEntry.has(reference.to) && !visited.has(reference.to)) { visited.add(reference.to); pending.push(reference.to); } return [...visited].sort(); }
