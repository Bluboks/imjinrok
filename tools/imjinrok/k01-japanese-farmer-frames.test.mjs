...
test("recovers class 31 source creation order, Farmerj identity, map records, and scoped core frames", () => {
  ...
  assert.deepEqual(report.creation, { mapLoader: "0x0048dbe0", wrapper: "0x00483c50", creator: "0x00437650", zeroFill: { dwordCount: 342, byteCount: 1368, instruction: "0x00437666" }, classWrite: "0x00437b86", initializerCall: "0x00437f29", initializerWordAtEntry: 0 });
  assert.deepEqual(report.initializerCalls, { initializerRange: "0x00429b90-0x00429c59", idle: { helper: "0x00428fb0", state: 8, slot: 145, frameStart: 0, phaseCount: 8, branch: "0x00429108" }, move: { helper: "0x00428e10", state: 1, slot: 145, frameStart: 160, phaseCount: 8, branch: "0x00428f1a" }, death: { helper: "0x004390b0", state: 7, slot: 145, frameStart: 240, frameStride: 0, phaseCount: 8, facingCallCount: 5 } });
  ...
  assert.deepEqual(fixture.vectors, [
    selectK01JapaneseFarmerFrame({ state: 8, direction: 1, phase: 0 }),
    selectK01JapaneseFarmerFrame({ state: 1, direction: 16, phase: 7 }),
    selectK01JapaneseFarmerFrame({ state: 7, direction: 65, phase: 7 }),
  ]);
});
...
test("rejects tampered canonical artifacts, raw bytes, map, and Farmerj sprite", (t) => {
  ...
  const mutateJson = (source, mutate) => { const path = join(temp, `${Math.random()}.json`); const value = JSON.parse(readFileSync(source, "utf8")); mutate(value); writeFileSync(path, `${JSON.stringify(value)}\n`); return path; };
