...
export const ACTION_GRID = Object.freeze({
  columns: 3,
  rows: 3,
  left: 525,
  top: 363,
...
const RAW_CODE_RANGES = [
  ...
  ["command-slot-getter", 0x00461200, 0x004612bd, "11e38c3ffdc3c65ce6872cefd068ebed74f75c4773ff0f75aa41a18ced2572ec"],
...
const EVIDENCE = [
  ...
  [0x00481eeb, "66 a1 2c 98 89 00 66 89 41 04 66 8b 15 30 98 89 00 b8 02 00 00 00 66 89 51 06", "initializer copies runtime signed WORD cell width and height from DAT_0089982c and DAT_00899830"],
  ...
  [0x0045b0c2, "43 83 c0 02 89 5c 24 10 3d 02 67 7c 00 89 44 24 14 0f 8c 37 fd ff ff", "the common renderer increments the index and two-byte cursor until the exclusive cursor 0x007c6702, producing exactly nine slots"],
  ...
  [0x0046120b, "66 85 c0 7d 49", "getter rejects a negative signed slot index before reading owner storage"],
  [0x00461259, "66 3d 09 00 7c 4a", "getter rejects an index greater than or equal to nine before reading owner storage"],
...
export function extractGameplaySelectionCommandPanel({ executablePath = DEFAULT_EXECUTABLE_PATH, referencesPath = DEFAULT_REFERENCES_PATH, panelPath = DEFAULT_PANEL_PATH, buttonPath = DEFAULT_BUTTON_PATH } = {}) {
...
export function reproduceGameplayCommandGrid(input) {
  ...
  const runtimeCellSize = assertRuntimeCellSize(input);
  output.runtimeCellSize = runtimeCellSize;
  const slots = assertSlots(input.slots);
  for (let slot = 0; slot < slots.length; slot += 1) {
    if (slots[slot].rendererEnabled === 0) continue;
    output.operations.push({ type: "draw-command-slot", slot, frameOrResourceIndex: slots[slot].frameOrResourceIndex, rect: commandSlotRect(slot, runtimeCellSize) });
  }
  ...
  const rect = commandSlotRect(slot, runtimeCellSize);
...
function assertRuntimeCellSize(input) { return { width: assertInteger(input.runtimeCellWidth, -32768, 32767, "runtimeCellWidth"), height: assertInteger(input.runtimeCellHeight, -32768, 32767, "runtimeCellHeight") }; }
