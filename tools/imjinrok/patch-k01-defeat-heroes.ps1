...
$recordPath = Join-Path $logDir "$Marker-k01-defeat-heroes-patch.json"
...
public static class CodexK01DefeatPatchMemory {
...
function Read-Memory {
  ...
  $read = [UIntPtr]::Zero
  $ok = [CodexK01DefeatPatchMemory]::ReadProcessMemory($Handle, [IntPtr]$Address, $buffer, [uint32]$Size, [ref]$read)
...
function Write-Memory {
  ...
  $ok = [CodexK01DefeatPatchMemory]::WriteProcessMemory($Handle, [IntPtr]$Address, $Bytes, [uint32]$Bytes.Length, [ref]$written)
...
$handle = [CodexK01DefeatPatchMemory]::OpenProcess($PROCESS_ACCESS, $false, [uint32]$targetPid)
...
try {
  ...
  for ($index = 0; $index -lt $recordCount; $index += 1) {
    ...
    Write-Memory $handle $recordAddress ([byte[]](0x00))
    Write-Memory $handle ($recordAddress + 1) ([byte[]](0xff))
    Write-Memory $handle ($recordAddress + 0x55) ([byte[]](0x00))
  ...
  $record = [ordered]@{
    ...
    purpose = "Force K01 current-owner hero records 0x4c/0x4e missing, then observe original executable defeat-timer code."
    targetPid = $targetPid
    currentOwner = $currentOwner
    target = $Target
  ...
  [CodexK01DefeatPatchMemory]::CloseHandle($handle) | Out-Null
}
