...
| `+0x0a` | word | 런타임 sprite slot | `FUN_0041fdb0` 소비, 클래스 2·49 범위 `정적 확정` |
| `+0x0c` | word | 최종 sprite frame | `FUN_0041fdb0` 소비, 클래스 2·49 범위 `정적 확정` |
...
| `+0x3c` | word | 최대 체력 | 클래스 49 체력 분기 범위 `정적 확정` |
| `+0x3e` | word | 현재 체력 | 클래스 49 체력 분기 범위 `정적 확정` |
| `+0x74` | dword | 타입·행동 flags | 클래스 2 방향 분기와 클래스 49 체력 분기 범위 `정적 확정` |
| `+0x8c` | byte | 건설 진행 정수 백분율 | 클래스 49 건설·체력 분기 범위 `정적 확정` |
| `+0x1b2` | word | 애니메이션 phase | 클래스 2·49 파일럿 범위 `정적 확정` |
| `+0x1b5` | byte | 좌우 mirror 선택자 | 클래스 2 파일럿 범위 `정적 확정` |
| `+0x1e6` | word | 일반 방향 값 | 클래스 2 상태 1 일반 분기·상태 2 범위 `정적 확정` |
| `+0x1e8` | word | 특수 방향 값 후보 | 클래스 2 상태 1 flags 분기에서 읽음, 매핑 미재현 |
| `+0x250` | word | 정상 0·반파 1 본체 상태 | 클래스 49 범위 `정적 확정` |
| `+0x456` | word | 프레임 소스 후보 | setter 경로 |
| `+0x48e` | word | 두 번째 프레임 소스 후보 | setter 경로 |
| `+0x4b2` | word | 건설 상태 sprite slot 설정 | 클래스 49 범위 `정적 확정` |
| `+0x4b4` | word | 건설 상태 frame offset | `typeBaseFrame - 7`, 클래스 49 범위 `정적 확정` |
...
- 좌표 필드의 좌표계와 화면 변환
- 클래스 2 상태 1의 `+0x1e8` 특수 방향 값 1000~1007 매핑
