...
- `FUN_00446420`의 결과 화면/상태 전환과 결과 code consumer
- 원본 result transition을 프로젝트 generic campaign 결과로 옮기는 identity/policy mapping

다음 좁은 질문은 K01 결과 진입·reset과 `FUN_00446420` 이후 결과 전환을 별도 정적 단위로
복원하는 것이다. 사용자가 제공한 “완성 봉화가 하나라도 있으면 미니맵 enable, 마지막 봉화 제거
