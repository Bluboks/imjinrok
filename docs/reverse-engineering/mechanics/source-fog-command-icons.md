...
| 구분 | 상태 | 범위 |
| --- | --- | --- |
| 분석 | 자원 identity는 정적 확정; fog frame 선택·mask 의미와 product action 대응은 미확인 | SHA-256, SPR header, button control frame/resource index 26..29 |
...
기존 [마법 자동사용 gate](magic-auto-use-gate.md)는 no-selection control의 action 61/62, CP949 labels `자동마법설정`/`자동마법해제`, frame/resource indices 27/26을 정적 확정했다. [hero-priority gate](hero-priority-queue-gate.md)는 action 63/64와 indices 28/29를 확정했지만 그 frame/resource indices의 pixel resource identity와 CP949 labels는 닫지 않았다.

현재 제품의 `ActionDefinitionId` 중 위 원본 control과 action constructor, CP949 label, frame/resource index를 모두 함께 만족하는 항목은 **없다**. 따라서 `sourceFogAndCommandAssets.ts`의 product-action map은 의도적으로 비어 있고, 현재 `actionGrid.ts`는 모든 제품 action에 기존 glyph fallback을 유지한다. frame/resource index 26..29가 `button.spr`의 pixel frame 26..29라는 complete path도 아직 없다. 그래서 exported PNG와 original control binding은 별도 record로 보존하며 서로를 semantic mapping으로 쓰지 않는다. source record가 후속 정적 분석으로 product action 및 pixel resource에 연결되면 action grid는 loaded texture를 image로 그리며, texture가 없으면 glyph로 조용히 대체하지 않고 오류를 낸다.
...
SkirmishScene는 이 문서 범위에서 변경하지 않았다. 후속 battlefield bridge는 resolver 결과와 `NORMAL_FOG_ASSETS`의 texture key/path를 preload·draw contract로 사용해야 하며, missing texture를 `requireSourceTexture`로 명시적으로 실패시켜야 한다.
...
남은 작업은 fog 96-frame scheduler와 original neighbor-mask/index rule의 static analysis, command frame/resource index가 `button.spr` pixel frame임을 보이는 complete path, 그리고 original control과 product action ID의 source-complete mapping이다.
