...
extractor는 기존 `codec.mjs` decoder/PNG encoder를 재사용한다. 원본 SHA와 header가 다르면 output을 만들기 전에 실패한다. export 범위는 `button.spr` frame `4,6,10,11,12,16,26,27,28,29,39,43,45` 및 각 normal fog source의 **frame 0 하나**다. button frame은 [command control의 pixel-frame 결합](command-icon-frame-binding.md)으로 닫힌 original control path만 export한다. fog의 96 frame 중 frame 0 이외의 선택 규칙은 정적으로 닫히지 않았으므로 내보내지 않았다. 이 범위는 asset identity 보존용이며 fog animation·terrain transition 규칙 주장도 아니다.
...
`imjinrok-k01-opening`만 `IMJINROK_SOURCE_COMMAND_ICON_PROFILE`을 opt in한다. profile은 아래처럼 source control record와 frame을 명시적으로 보존한다.

| 제품 action | original action·label | frame | 제품 판정 |
| --- | --- | ---: | --- |
| `move` | 3, `이동` | 6 | exact source control binding |
| `stop` | 2, `정지` | 43 | exact source control binding |
| `patrol` | 35, `순찰` | 10 | exact source control binding |
| `repair` | 16, `수리` | 12 | exact source control binding |
| `hold` | 39, `사수` | 39 | exact source control binding |
| `rally-point` | 21, `집결지설정` | 11 | exact source control binding |
| `cancel-production`, `cancel-construction` | 19, `취소` | 45 | exact source control binding |
| `attack-move` | 5, `공격` | 4 | source-backed adaptation; product attack-move semantics differ |
| `build` | 11, `건설` | 16 | source-backed adaptation; product build semantics differ |

profile이 없는 generic/mod action grid는 glyph fallback을 유지한다. profile이 binding을 제공하면 `actionGrid.ts`는 loaded texture를 image로 그리며, registered source texture가 없으면 glyph로 조용히 대체하지 않고 오류를 낸다. 이 error boundary는 source asset을 opt in한 profile에만 적용한다.
