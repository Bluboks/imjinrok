...
| `0x00421c50`, `0x00438930`, `0x00438aa0` | building renderer, screen-rect pivot helpers | selector jump table; dimension/flag split | building class `41..95` 2단계 selector→destination dispatch, current-SPR pivot split, shared effect state `+0x550/+0x552/+0x554` | [원본 엔티티 전수 시각 프로필](mechanics/original-entity-visual-profiles.md)에서 table hashes와 conditional quarantine를 재현 |
...
| `0x00445290`, `0x00444770`, `0x00444570`, `0x004446a0`, `0x0043ab70` | bounded source pathfinding wrapper/core/candidate/postprocess/footprint predicate | 58; 383; 58; 66; 75 instructions | accepted-node gate, Chebyshev open-frontier capacity, strict squared-goal frontier/tie, exact eight-state candidate vector, footprint mask/OOB and fallback postprocess call | terrain-mask producer semantics, workspace lifecycle/serialization, raw waypoint/product coordinate and full navigation lifecycle remain 미확인 |
...
| `0x0048dbe0`, `0x00483c50`, `0x00437650`, `0x0043c9c0`, `0x0043ad30` | K01 opening footprint anchor | 147; 26; 539; 684; 399 | map source index→runtime slot/generation→type `+0x14/+0x16` footprint copy→action 1 writer | class 48/49/50/51/57/58/60/62/63 active records, centered `floor(extent/2)` anchor, per-cell OOB skip, mask OR then slot-owner store 정적 확정; [K01 opening footprint anchor](mechanics/k01-opening-footprint-anchor.md) |
...
| `0x00425af0`, `0x00425b20` | `FUN_00425af0`, `FUN_00425b20` | ? / 11; ? / 506 | K01 normal movement wrapper와 next-tile/coordinate commit | mask/path gate, progress `0x32` 이상에서만 `+0x1bc/+0x1be` commit 정적 확정; [occupancy-owner transition](mechanics/k01-occupancy-owner-transition.md) |
| `0x0043ac50` | `FUN_0043ac50`, `0x0043ac50-0x0043ad21` | ? / 74 | footprint mask consumer | `0x00ae27e4`와 supplied mask의 any-nonzero를 blocked로 반환, OOB cell skip 정적 확정 |
...
| `0x0043b2d0` | `FUN_0043b2d0`, `0x0043b2d0-0x0043b4cd` | ? / 157 | mobile prior owner/mask clear | action-1 선행 clear와 release clear, active-gate/footprint OOB 경계 정적 확정 |
| `0x0043c300` | `FUN_0043c300`, `0x0043c300-0x0043c9b1` | ? / 524 | action-1 create-return direct helper | `+0x1bc/+0x1be`를 읽고 rewrite하지 않는 K01 immediate scope 정적 확정. class 50 action-1 primary path는 active·`(+0x74&0x01000040)==0`·signed-absolute tick delta `>=5`에서 state 8과 signed-IDIV `+0x1b2` remainder를 기록하며 class-50 divisor `+0x92=1`로 phase 0을 유지 |
| `0x0043c9c0` | `FUN_0043c9c0`, `0x0043c9c0-0x0043d35f` | 143 / 684 | `+0x1b0` 상위 행동 dispatcher와 outer keep/release 반환 | K01 action 1 call edge `0x0043b2d0→0x0043c300→0x0043ad30`, 상태 5 공격·클래스 76·78 health-zero 행동 6/7/`0x16` 정적 확정. class 50 constructor action 1은 case `0x0043cdaa`로 이 primary producer를 reached |
| `0x00445290`, `0x0043d540` | movement path/postprocess helpers | ? / 58; ? / 71 | normal movement path gate와 coordinate postprocess | blocked/failure return이 coordinate commit보다 선행·후속 call edge 정적 확정 |
| `0x004651b0` | `FUN_004651b0`, `0x004651b0-0x00465206` | ? / 25 | occupancy mask cell clear helper | in-bounds cell에서 `WORD &= 0x0fff`, OOB/invalid map skip 경계 정적 확정 |
...
| `0x004576c0` | `FUN_004576c0`, `0x004576c0-0x004576f9` | 1 / raw range | UI control record constructor | argument action WORD→record `+0x00`, frame/resource WORD→`+0x02`; action 2/3/5/11/16/19/21/35/39의 CP949 runtime label pointer와 button frames, action 61/62/63/64 frames 결합 정적 확정 |
...
| `0x00464cc0`, `0x00469330`, `0x00469510`, `0x0046d650` | `FUN_00464cc0`, `FUN_00469330`, `FUN_00469510`, `FUN_0046d650` | ? / 103; ? / 150; ? / 341; ? / 50 | K01 signed x/y→`(x-y)<<5`/`(x+y)<<4`, `+0x4a0c4` fog-family indexed WORD table and `+0x32514` low-nibble/helper relative branch→`+0x3a3a4/+0x42234` object/frame bytes; corrected `FUN_0046d650` lookup is `map+0x51f54+selector*0x7e90+x*180+y` after the final `LEA ... *4` | K01 corrected helper return `0:1639,1:616,2:1115,3:74,4:156`; raw shift `0:1331,16:617,32:1335,48:128,64:189`. `DAT_00c06e86` table values/semantics, height human meaning, axis/pivot and renderer parity are 미확정; [placement evidence fixture](../../analysis/fixtures/k01-tile-placement-elevation-evidence.json) fixes the corrected stream digests |
| `0x004648e0`, `0x004653d0`, `0x00481c50` | `FUN_004648e0`, `FUN_004653d0`, `FUN_00481c50` | ? / 315; ? / 108; ? / 199 | `FUN_004648e0`은 `FUN_00464cc0` outputX `0x00843984+4*(x*180+y)`/outputY `0x008633c4+4*(x*180+y)`에 write; `FUN_004653d0`은 returned pair squared-distance compare; `FUN_00481c50`은 nested loop에서 y-only output pair를 overwrite | `FUN_00481c50`은 cell별 table writer가 아니다. corrected outputY joint와 raw shift는 [K01 tile placement boundary](mechanics/k01-tile-placement-elevation-boundary.md)를 따른다. terrain/elevation·axis semantics는 미확정 |
...
| `0x0044cc20` | `FUN_0044cc20`, `0x0044cc20-0x0044cccb` | ? / 72 | gameplay compositor의 indexed target/strip clear helper | `FUN_004676e0` default path에서 clear index 0과 full target/strip fill 범위 정적 확정; 다른 caller의 clear surface·lifecycle은 미확정 |
| `0x00454250` | `FUN_00454250`, `0x00454250-0x004542dd` | ? / 51 | default YTL row payload blitter | row skip/count와 payload byte direct copy, index 0 opaque·`0xfe` export sentinel 및 clear-gap 보존을 gameplay fixture로 재현; blend/alpha와 auxiliary caller semantics는 미확정 |
| `0x004676e0` | `FUN_004676e0`, `0x004676e0-0x00467ddc` | ? / 507 | bounded gameplay terrain compositor: `640×384` target, y-outer/x-inner camera traversal, default normal-tile path | clip `0..639/0..383`, `FUN_0044cc20` index-0 clear, `FUN_00469510` direct call edge와 default payload vector를 정적 확정; auxiliary/nondefault renderer paths, broader pivot/axis와 full renderer parity는 미확정 ([gameplay compositor fixture](../../analysis/fixtures/k01-gameplay-terrain-compositor.json)) |
| `0x00467de0` | `FUN_00467de0`, `0x00467de0-0x004689ce` | ? / 901 | state `4`/`8`별 8-neighbor fog mask→`0x004bf9c4` lookup→compositor | two literal-state paths, nonzero/non-`0x0f` gate, lookup and `FUN_0046a530` call are 정적 확정; bit/visibility semantics are 미확정 |
| `0x0046a530` | `FUN_0046a530`, `0x0046a530-0x0046a8e8` | ? / 299 | `map+0x4a0c4+x*180+y` family byte+`0x3c`→common fog record, state별 3×2 compositor | fog records 60..74, black 75, K01 hash-bound map byte distribution, normal fog `atlasColumns=32`/`halfColumns=16` signed-IDIV frame algebra, two call paths and six subframes are 정적 확정; producer/lifetime, scheduling/pixel placement are 미확정 |
| `0x00469510` | `FUN_00469510`, `0x00469510-0x00469916` | ? / 341 | default normal-tile selector and direct YTL row dispatch | fixed first-argument subtract, helper-derived second-argument subtract, local mode/clip gate와 `FUN_00454250` direct payload path를 K01 gameplay fixture로 재현; auxiliary/nondefault caller axis/pivot, broader mode/clip semantics와 full renderer parity는 미확정 |
| `0x00447360` | `FUN_00447360`, `0x00447360-0x00447599` | 35 / 156 | active entity list 뒤 투사체 slot 0~99 순회 | entity dispatcher 반환 0→`0x00483aa0`, subtype `0x0c` pool 범위와 K01 outer release edge 정적 확정. class 50 primary phase scope에서는 active-list `slot*0x558+0x00635258`→`0x0043c9c0` caller edge 정적 확정 |
| `0x00447bc0` | `FUN_00447bc0`, `0x00447bc0-0x00447cfa` | 21 / 75 | 원본 simulation-step scheduler | pre-update `FUN_004464c0`·wall-clock gate 뒤 모든 gate 승인 시 `FUN_00447360` 1회, K01 result wrapper는 pool보다 선행하는 raw 조건·순서 정적 확정; callee 의미 미확정 |
| `0x0048ea90` | `FUN_0048ea90`, `0x0048ea90-0x004924b2` | command label string copier | `0x00aa4018` base의 fixed runtime label buffers에 CP949 source string을 length scan·`REP MOVS`로 copy | action 2/3/5/11/16/19/21/35/39의 source label→runtime pointer→constructor binding 정적 확정; product semantics는 미확정 |
...
| `0x00483aa0` | `FUN_00483aa0`, `0x00483aa0-0x00483c2e` | 12 / 95 | active-list swap-last 제거와 active/reuse WORD clear | action-7 반환 0 뒤 clear helper→list cleanup→active/reuse clear 순서, active-gate 실패 시 stale occupancy 가능성 정적 확정 |
