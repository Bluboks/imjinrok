...
| `0x00464cc0`, `0x00469330`, `0x00469510`, `0x0046d650` | `FUN_00464cc0`, `FUN_00469330`, `FUN_00469510`, `FUN_0046d650` | ? / 103; ? / 150; ? / 341; ? / 50 | K01 signed x/y→`(x-y)<<5`/`(x+y)<<4`, `+0x4a0c4` fog-family indexed WORD table and `+0x32514` low-nibble/helper relative branch→`+0x3a3a4/+0x42234` object/frame bytes | K01 3,600 cells: helper lookup/result 0; raw product-adapter stream is 2,865×0/735×16. `DAT_00c06e86` table values/semantics, height human meaning, axis/pivot and renderer parity are 미확정 |
| `0x004648e0`, `0x004653d0`, `0x00481c50` | `FUN_004648e0`, `FUN_004653d0`, `FUN_00481c50` | ? / 315; ? / 108; ? / 199 | `FUN_004648e0`은 `FUN_00464cc0` outputX `0x00843984+4*(x*180+y)`/outputY `0x008633c4+4*(x*180+y)`에 write; `FUN_004653d0`은 returned pair squared-distance compare; `FUN_00481c50`은 nested loop에서 y-only output pair를 overwrite | `FUN_00481c50`은 cell별 table writer가 아니다. K01 outputY `+16/+9` joint와 raw raster `0/16`은 별개이며 terrain/elevation·axis semantics는 미확정 |
