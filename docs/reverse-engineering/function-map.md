...
| `0x004291d0` | `FUN_004291d0` | 클래스 점프 테이블 포함 | 내부 클래스별 애니메이션 설정 초기화 | class 7→`0x0042981d`의 state 8/1 helper와 five-facing state 7 frame 240, class 13→`0x0042a492`, class 82→`0x0042ae03` 상태 8/1/4/7, class 14→`0x0042bae1` 상태 8/1/4, class 76→`0x0042a9da`, 78→`0x0042ab2a` 범위 정적 확정 |
| `0x00428fb0`, `0x00428e10` | `FUN_00428fb0`, `FUN_00428e10` | class switch / 148; class switch / 86 | state 8 idle·state 1 move setup helper | class 7 case의 `+0x47a==0` branch가 각각 state 8·start 0·slot 105·phase 8→`0x00438e50`, start 40·slot 105·phase 8→`0x00438ef0`을 정적 확정; nonzero branches는 별도 |
...
| `0x00437650`, `0x004550c0` | `FUN_00437650`, `FUN_004550c0` | 39 / 539; 1 / 26 | `0x558`-byte 엔티티 초기화와 `+0x45c` 0x38-byte subrecord clear | K01 map loader→wrapper→creator가 source class 7을 생성할 때 `REP STOSD` zero와 `+0x45c` callee zero 뒤 initializer를 호출해 initializer-time `+0x47a==0`을 정적 확정; class 12/13/14/82 create-return·이후 runtime mutation은 별도 |
