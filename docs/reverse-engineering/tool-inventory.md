...
| `extract-k01-reinforcement-identity-map.mjs`, `k01-reinforcement-identity-map.test.mjs` | 유지 | 기존 native descriptor, 타입 카탈로그·K01 map·네 SPR와 current sprite audit provenance를 교차 검증해 요청 좌표 9/9, exact static identity/source 3/9, proxy 6/9를 분리; 최종 배치·행동·stats·animation parity는 포함하지 않음 |
