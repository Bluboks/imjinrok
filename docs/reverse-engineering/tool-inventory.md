...
| `extract-command-icon-frame-binding.mjs`, `command-icon-frame-binding.test.mjs` | 유지 | common loader index 18의 `button.spr` record/payload·offset table과 `FUN_004576c0` action/frame WORD writer, action 61/62/63/64→pixel frame 27/26/28/29, `FUN_0045ad90`의 `34×34` selected-command draw path를 canonical EXE/SPR/references hash·raw range·anchor로 검증한다. disabled slot·loader failure·out-of-range는 successful draw를 만들지 않으며 product semantic mapping은 제외한다. |
