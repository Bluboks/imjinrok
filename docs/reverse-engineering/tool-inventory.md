...
| `extract-k01-reinforcement-identity-map.mjs`, `k01-reinforcement-identity-map.test.mjs` | 유지 | native descriptor, 타입 카탈로그·K01 map·네 SPR, 네 conversion manifest와 current sprite audit/K01 adapter provenance를 교차 검증해 요청 좌표·exact static identity/source 9/9를 확정; 최종 배치·행동·stats·animation은 포함하지 않음 |
