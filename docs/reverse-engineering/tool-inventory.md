...
| `extract-k01-class2-locomotion-bridge.mjs`, `k01-class2-locomotion-bridge.test.mjs` | 유지 | class 2 initializer의 computed zero-fill과 `+0x4ea/+0x4ee/+0x4f2` direct writer set, normal-entry selector-1 third reduction·negative-to-positive clamp, signed accumulator threshold의 recovered current-coordinate pair commit을 canonical EXE/raw anchor와 boundary vector로 추출·재현한다. coordinate unit, occupancy, scheduler cadence와 product 24 Hz/FPS mapping은 포함하지 않는다. |
| `extract-k01-source-coordinate-bridge.mjs`, `k01-source-coordinate-bridge.test.mjs` | 유지 | K01 map cell·exact placement·class-2 locomotion current/raw/accumulator·Ryu projectile start/end/route를 signedness/width/unit/anchor별로 분리하고, map/placement의 닫힌 in-bounds subset만 semantic `GridPoint` identity 양방향 계약으로 제안한다. C01 accepted-step·E01 source-handle linkage는 확인 범위만 연결하며, universal scale·pixel/visual evidence·locomotion/projectile mapping·production adapter는 주장하지 않는다. |
...
| `extract-k01-hero-aura-evidence.mjs`, `k01-hero-aura-evidence.test.mjs` | 유지 | class 76/78 type initializer와 `eventmark.spr` resource-table candidate의 canonical input hash·anchor만 고정한다. producer/recipient CFG, stat/radius/timing 및 resource frame/compositor는 미확인으로 명시하며 원본 aura를 주장하지 않는다. |
...
| `extract-k01-clock-mode-producers.mjs`, `k01-clock-mode-producers.test.mjs` | 유지 | state-5 record `+8`→local→`EBX`→mode writer, selector setter·base/feedback gate와 guard direct-write/serialization-address boundary를 EXE·generated evidence에 결합해 재현한다. mode 1의 조건부 `50 ms` base만 확인하며 K01 stage-1 reachability, exact mode/selector·tick frequency와 serialization alias effect는 주장하지 않는다. |
| `extract-k01-clock-selector-initialization-evidence.mjs`, `k01-clock-selector-initialization-evidence.test.mjs` | 유지 | main cold-start의 `config.hq` open-failure→`0x00634ab8` initializer→selector DWORD 2와 successful `0x1d4` transfer의 `+0x14` coverage를 EXE/functions/references/string/raw anchor와 load-failure/load-success vector로 재현한다. later option mutation, scheduler mode/session reachability, feedback와 fixed cadence는 주장하지 않는다. |
...
| `extract-original-entity-visual-profiles.mjs`, `original-entity-visual-profiles.test.mjs` | 유지 | 타입 1~95의 numeric flags·signed `+0x0c` pivot·case-insensitive canonical SPR header/hash·2단계 building selector table·construction/body/overlay profile을 추출하고 class-41 conditional draw를 `conditionalEffects`로 quarantine한다. generated JSON/TS/fixture는 실행 파일·table hash에 묶이며 runtime tick/elevation/FPS 의미는 주장하지 않는다. |
...
| `map-data-protocol.mjs`, `map-data-protocol.test.mjs` | 유지 | 선언형 protocol v1이 MAP/EXE 해시·헤더·extent를 검증하고 field_0x32514/object/frame/fog/placement/corrected raw shift/passability 채널을 JSON·browser-safe TS로 결정론 생성한다. `K01_TERRAIN_RLE`은 원본 채널에서 제외한다. |
| `extract-k01-tile-placement-elevation-evidence.mjs`, `k01-tile-placement-elevation-evidence.test.mjs` | 유지 | `FUN_00462b80`의 15-entry 8-byte-stride WORD initializer(family 0=`0`, 1..14=`9`, adjacent zero WORD), `FUN_00464cc0`/`FUN_00464ea0` 두 reader와 `DAT_00c06e86`의 exact six direct-reference inventory, signed x/y→`(x-y)<<5`/`(x+y)<<4` projection, corrected `FUN_0046d650` lookup `map+0x51f54+selector*0x7e90+x*180+y`, low-nibble/helper branch, `FUN_00466f20` source full-raster y-outer/x-inner order를 K01 3,600-cell stream과 initializer/reader vector로 재현한다. alias/computed writer, lifetime/order와 table·helper의 사람용 height 의미, broader pivot/axis, auxiliary/nondefault clip/mode/palette와 full renderer parity는 주장하지 않는다. default gameplay viewport의 clip/mode-0/index-0 clear/palette entry-0은 별도 `extract-k01-gameplay-terrain-compositor` fixture/test가 담당한다. |
| `extract-k01-cell-projection-evidence.mjs`, `k01-cell-projection-evidence.test.mjs` | 유지 | `FUN_004648e0`의 x-major per-cell outputX/outputY write와 `FUN_00481c50`의 y-only row overwrite call loop, `FUN_004653d0` returned-coordinate squared-distance consumer, six direct `FUN_00464cc0` call edges 및 corrected K01 low-nibble/family/runtime-WORD/helper/outputY joint stream을 고정한다. outputY joint와 raw shift는 [K01 tile placement boundary](mechanics/k01-tile-placement-elevation-boundary.md)를 따르며 terrain height/elevation 의미는 주장하지 않는다. |
| `extract-source-fog-render-evidence.mjs`, `source-fog-render-evidence.test.mjs` | 유지 | `FUN_00467de0` state 4/8 mask·lookup, caller projection과 six-argument order, `FUN_0046a530` family record·low-nibble vertical branch, `64×48` six-subframe local `(32,0)` anchor와 corrected K01 raw-shift domain을 EXE·SPR·K01 map hash-bound vector로 추출·재현한다. state/visibility mapping, alpha/tint, viewport ownership, renderer-wide pivot·clip·mode와 web scheduling은 주장하지 않는다. |
| `extract-k01-gameplay-terrain-compositor.mjs`, `k01-gameplay-terrain-compositor.test.mjs` | 유지 | `FUN_004676e0`의 `640×384` target·`0..639/0..383` clip·y-outer/x-inner traversal·`FUN_0044cc20` index-0 clear, `FUN_00469510→FUN_00454250` direct YTL row payload semantics, five palette entry-0 RGB(0,0,0), 243-frame payload stats와 camera `(0,0)` 2×2 crop digest/pixel vector를 EXE·YTL·PAL·K01 map hash-bound fixture로 추출·재현한다. source placement offset은 physical elevation과 분리하며, broader auxiliary paths와 renderer-wide parity는 주장하지 않는다. |
| `extract-source-fog-visibility-lifecycle.mjs`, `source-fog-visibility-lifecycle.test.mjs` | 유지 | `FUN_00460ba0`의 180×180 literal-8 initialization, `FUN_004610e0` mode/raw-`+0x1ec` age branch와 dirty reset, `FUN_00439260` local identity/selector/one-shot gate, `FUN_00442ca0` selector 0..11 bounded nonzero→0/dirty-1 pattern write를 EXE·canonical artifacts·sparse vectors로 추출·재현한다. `+0x1ec` human meaning, selector/type provenance, scheduler 및 render alpha/tint/palette/blend는 주장하지 않는다. |
