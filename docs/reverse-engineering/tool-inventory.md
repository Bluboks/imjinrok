...
| `extract-k01-tile-placement-elevation-evidence.mjs`, `k01-tile-placement-elevation-evidence.test.mjs` | 유지 | `FUN_00464cc0` signed x/y→`(x-y)<<5`/`(x+y)<<4` projection, `+0x4a0c4` byte-indexed unresolved WORD-table boundary, `FUN_00469330`/`FUN_00469510` `+0x32514` low-nibble branch와 `FUN_0046d650` selector/lookup return, `FUN_00466f20` source full-raster dimensions·y-outer/x-inner order·caller argument/call edge를 K01 3,600-cell stream으로 재현한다. helper/table height meaning, broader pivot/clip/palette와 full product renderer parity는 주장하지 않는다. |
