...
| 전투·피해 | K01 일반 공격과 class 78 action 40, action 59 loop-carried full DWORD·fixed reset/tracking·subtype 16 helper-zero 후보/same-slot subtype 1 전환·mode 2 kind 2, generic mode 1 kind 2 ring 열거·live/dedup·payload·current-reference callback 입력 경계 및 subtype 12 kind 9 산식·low-active/class-95/direct-generation-mismatch/writer gate 정적 확정 | 일반 공격 제한 범위와 class 78 delivery·subtype 12/16 flight/종료·generic enumeration 입력/call 경계·선택 buffer/health write 부분 재현 | 프로토타입, 자동 마법은 미이식 | generic consumer 이후 callback whole result, K01 mode 1 producer, 재경로 좌표 결과, death/reference invalidation, 대상 검색·사거리, tick 초 환산 |
...
| UI·입력 | 기존 범위에 더해 class 78 auto origin 1과 manual origin 0의 pending-store 우선순위·일반 공격 short-circuit, 9-slot command-grid의 successful `button.spr` `34×34` source binding·strict edge, `pannel.spr` `640×163` `(0,0)` locked blit과 grid 선행 order 정적 확정 | class 78 자동 action 40/59 record와 manual-block/replace·source-invalid·no-op 경계 부분 재현; command-grid exact rectangle/strict edge와 loader·blit gate failure 재현 완료 | K01 `imjinrok-k01-opening`의 9-slot geometry·strict hit는 원본 기반; selection panel의 반응형·다중 선택·mana·추가 상태와 action label/semantics·HUD background placement는 의도적 프로젝트 UI | remembered right-click reservation action/owner, 나머지 8개 magic-auto class effect, outer input/entity update 순서, computed-alias writer 부재·selected renderer contents·runtime surface semantic owner |
