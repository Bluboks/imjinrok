...
| 이동·경로 탐색 | `FUN_00425b20→FUN_00445290→FUN_00444770`의 accepted-node gate, Chebyshev open-frontier capacity, strict squared-goal greedy selection/tie, ±25 window, exact eight-state candidate vector, footprint mask/OOB predicate, closest fallback·postprocess call 범위와 class 2 type `+0x3c/+0x50`→entity `+0x4ee/+0x4ea` 초기화, `+0x4ee→+0x4f2` raw accumulator read/write 경로 정적 확정; workspace lifecycle/terrain semantics/product coordinate와 raw field의 속도·변위 단위는 미확인 | bounded synthetic goal/fallback/block/OOB/cap/failure 및 class 2 raw field/cadence boundary vectors 재현 완료 | `imjinrok:source-greedy-local-adapter`는 bounded kernel을 K01 metadata-selected full-path product adapter로 연결한 `source-backed-adaptation`; collision/goal adaptation/chaining/전체 accepted-node budget는 project policy이고 `core:a-star`는 기본값. 제품 `movementSpeed`와 24 Hz displacement는 project fallback으로 유지 | mask producer/terrain semantics, workspace reset/serialization, postprocess output/product coordinate, `+0x4ee/+0x4ea` 후속 writer, full movement lifecycle와 original update→24 Hz conversion |
