...
| 엔티티 자료구조 | source handle lifecycle 범위 정적 확정; 전체 구조체는 추정 | allocator/generation/validity/release 범위 재현 완료 | A02가 K01 profile v2의 bounded `entityRuntime`/`occupancy` SSOT와 opening/admission adapter를 부분 이식; 전체 source mega-struct parity는 주장하지 않음 | 전체 mega-struct·모든 runtime writer/alias·identity mapping 교차 확인 |
...
| K01 occupancy-owner transition | native allocate→record init→action-1 clear/helper/writer, mobile owner `0x00ac2da4` slot overwrite, mask `0x00ae27e4`, movement `0x32` coordinate commit, outer release/death retain·stale-clear 경계를 정적 확정 | 20 vector replay, function/anchor/call-edge hash, root-independent path와 EXE/artifact/fixture tamper rejection 재현 완료 | A02는 generation 없는 slot-WORD owner cell과 엄격한 admission collision/OOB boundary만 production SSOT에 연결; source movement overwrite/scheduler는 연결하지 않음 | `0x00ad2ac4` alternate alias, raw owner/player 의미, K01 밖 footprint, computed/alias writer와 scheduler serialization |
| K01 opening footprint anchor | type writer argument 8/9→type `+0x14/+0x16`, creator copy→runtime `+0x1e3/+0x1e4`, map loader source-index order→wrapper generation/slot→action-1 writer, centered `floor(extent/2)` anchor, mask-OR→slot-owner store와 per-cell OOB skip, class 48/49/50/51/57/58/60/62/63의 15 opening records 정적 확정 | odd/even/mixed extents, class 49 `(5,4)` 3×3, class 7 `(7,6)` 1×1 control, edge/OOB, overwrite, active-gate·pre-copy와 artifact tamper 8-vector 재현 완료 | 분석 전용(`analysis-only-no-production-change`); sprite/pivot/project coordinate adapter 미연결 | raw owner/player 의미, alternate/non-mobile writer, construction/damaged/overlay/timing/pivot/stats/commands/behavior, scheduler serialization |
