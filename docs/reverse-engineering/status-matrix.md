...
| 엔티티 자료구조 | source handle lifecycle 범위 정적 확정; 전체 구조체는 추정 | allocator/generation/validity/release 범위 재현 완료 | T01 source runtime v3가 bounded `entityRuntime`/`occupancy` SSOT와 opening/class-52 admission adapter를 부분 이식하고 semantic-unit removal hook이 기존 release를 소비함; construction-time stale-owner bridge는 현재 semantic blocker와 unknown owner를 보존하며 proven stale owner만 복사본에서 제거함 | 전체 mega-struct·모든 runtime writer/alias·identity mapping·원본 death timing 교차 확인; legacy 1×1 saved beacon raw record migration은 수행하지 않음 |
...
| K01 봉화대 트리거 | raw-relation blocker→1,200-slot 완성 record scan→flag·K0120·native effect→post-state 반환과 descriptor slot/OOB/exact create·1×1 occupancy overwrite 정적 확정 | trigger 분기·descriptor·selector·return, slot/경계/overlap/WORD-wrap 및 타입/SPR·요청 좌표 9개 재현 완료; objective projection·remove/rebuild/save vectors의 focused 검증은 source-removal `43/43`·scenario-policy `142/142`와 독립 blocked/remove/rebuild/save sequence가 통과했고, 불필요한 기존 테스트 정리 전 전체 `pnpm test`도 `1,449/1,449` 통과 | source runtime v3의 completed-beacon/native admission과 K01 beacon/scenario policy가 source trigger·native effect를 project objective boundary에 연결; K01 adapter 9개 exact static identity/source와 in-bounds exact request-position create를 소비 | class 12 state-2·class 14 generic Facing/runtime tick, 네 class stats/behavior, raw owner, source scheduler·movement/death/result lifecycle |
| K01 occupancy-owner transition | native allocate→record init→action-1 clear/helper/writer, mobile owner `0x00ac2da4` slot overwrite, mask `0x00ae27e4`, movement `0x32` coordinate commit, outer release/death retain·stale-clear 경계를 정적 확정 | 20 vector replay, function/anchor/call-edge hash, root-independent path와 EXE/artifact/fixture tamper rejection 재현 완료 | source runtime v3는 generation 없는 slot-WORD owner cell과 엄격한 admission collision/OOB boundary를 production SSOT로 사용하고 semantic-unit removal은 기존 release를 호출; source movement overwrite/scheduler/death timing은 연결하지 않음 | `0x00ad2ac4` alternate alias, raw owner/player 의미, K01 밖 footprint, computed/alias writer와 scheduler serialization |
| K01 opening footprint anchor | type writer argument 8/9→type `+0x14/+0x16`, creator copy→runtime `+0x1e3/+0x1e4`, map loader source-index order→wrapper generation/slot→action-1 writer, centered `floor(extent/2)` anchor, mask-OR→slot-owner store와 per-cell OOB skip, class 48/49/50/51/57/58/60/62/63의 15 opening records와 dynamic class 52의 3×3 initializer 정적 확정 | odd/even/mixed extents, class 49 `(5,4)` 3×3, class 7 `(7,6)` 1×1 control, class 52 vector, edge/OOB, overwrite, active-gate·pre-copy와 artifact tamper 9-vector 재현 완료 | source runtime opening seed와 completed-beacon adapter가 source-confirmed footprint를 소비하며, shared resolver가 source-center extent를 placement·collision·range·build work·client geometry에 전달; sprite/pivot/native cell-cache/full movement는 별도 경계 | raw owner/player 의미, alternate/non-mobile writer, construction/damaged/overlay/timing/pivot/stats/commands/behavior, scheduler serialization, legacy 1×1 saved beacon migration |
