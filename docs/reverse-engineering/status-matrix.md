...
| 원본 fog renderer 경계 | `FUN_00467de0` state 4/8 별도 8-neighbor mask→`0x004bf9c4` lookup, `FUN_0046a530` family byte+`0x3c`→fog record 60..74와 black 75, state별 3×2/six-subframe call path 정적 확정 | hash/header·lookup 16-vector·record address·loop-bound 재현 완료 | 없음 | state 사람용 의미, family-byte producer, exact six-frame index algebra·scheduler·pixel placement와 제품 mapping |
