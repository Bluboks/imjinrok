...
| 이동·경로 탐색 | action 1 dispatcher의 old-grid helper→movement helper→writer 순서와 mobile writer의 no-occupant-test 범위만 정적 확인; destination coordinate/failure branch는 미확인 | 미재현 | 프로젝트 전용 generic footprint occupancy·stable reservation | `FUN_0043c300`의 목적지 좌표·occupied destination 분기와 caller 범위 복원 |
