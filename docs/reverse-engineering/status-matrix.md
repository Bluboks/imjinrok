...
| 원본 building far-cell placement와 product geometry adapter | `FUN_00438930`/`FUN_00438aa0`의 signed center/footprint fields, far occupied-cell formula, separate SPR slot pixel dimensions `+0x1da/+0x1dc`, pivot branches, cached projection consumers와 draw-field writes를 정적 확정. `+0x1e3/+0x1e4`는 SPR pixel dimensions가 아님 | EXE·function raw range·call-edge·opcode anchor·representative SPR hash와 3x3/3x2/2x2/1x1·signed offset·pivot/cached projection vectors를 bounded replay 완료 | product building-only adapter가 common actual interaction tiles와 elevation-aware far contact/polygon을 사용하고 semantic center container·local sprite offset·sprite bounds hit tests·derived depth를 소비; native cell-cache addition/native footprint extents parity는 주장하지 않음 | native table producer/lifetime·camera transform, full native cell-cache usage, source/player occupancy semantics, construction/compositor parity |
...
| 엔티티 자료구조 | source handle lifecycle 범위 정적 확정; 전체 구조체는 추정 | allocator/generation/validity/release 범위 재현 완료 | T01 source runtime v3가 bounded `entityRuntime`/`occupancy` SSOT와 opening/admission adapter를 부분 이식하고 semantic-unit removal hook이 기존 release를 소비함; 전체 source mega-struct·원본 death parity는 주장하지 않음 | 전체 mega-struct·모든 runtime writer/alias·identity mapping·원본 death timing 교차 확인 |
...
| K01 opening footprint anchor | type writer argument 8/9→type `+0x14/+0x16`, creator copy→runtime `+0x1e3/+0x1e4`, map loader source-index order→wrapper generation/slot→action-1 writer, centered `floor(extent/2)` anchor, mask-OR→slot-owner store와 per-cell OOB skip, class 48/49/50/51/57/58/60/62/63의 15 opening records 정적 확정 | odd/even/mixed extents, class 49 `(5,4)` 3×3, class 7 `(7,6)` 1×1 control, edge/OOB, overwrite, active-gate·pre-copy와 artifact tamper 8-vector 재현 완료 | source runtime opening seed가 source-confirmed footprint를 제한적으로 소비; sprite/pivot/project coordinate adapter와 일반 product occupancy는 미연결 | raw owner/player 의미, alternate/non-mobile writer, construction/damaged/overlay/timing/pivot/stats/commands/behavior, scheduler serialization |
...
| K01 source coordinate bridge | map cell과 exact placement의 signed-WORD·x-major·bounds·1x1/exact pair 및 class-2 raw/current/accumulator·Ryu start/end/route signed-WORD 범위를 domain별로 분리하고, opening building의 source footprint anchor/width/height는 [opening footprint anchor](mechanics/k01-opening-footprint-anchor.md)에서 별도로 닫음. C01 accepted scheduler→entity/projectile 순서와 E01 slot/generation/coordinate linkage는 확인 범위만 연결; locomotion/projectile scale·project anchor·24 Hz/FPS·identity는 미확정 | source-word/map-edge/placement nine-pair/accumulator threshold·signed boundary/projectile accepted route 및 opening-footprint 8-vector·artifact tamper 재현 완료 | 분석 전용; production package/app 변경 없음 | locomotion/projectile source unit·producer/consumer·GridPoint scale·project coordinate adapter, scheduler clock and project identity mapping |
