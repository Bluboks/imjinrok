...
| 원본 fog renderer 경계 | `FUN_00467de0` state 4/8 별도 8-neighbor mask→`0x004bf9c4` lookup, `FUN_0046a530` `map+0x4a0c4+x*180+y` family byte+`0x3c`→fog record 60..74와 black 75, K01 map byte domain/distribution, normal fog header `32×16`/96/`1024×48`의 signed-IDIV frame algebra·state별 3×2/six-subframe call path 정적 확정 | map/hash·header·atlas field·lookup 16-vector·family stream·record address·frame vector·loop-bound 재현 완료 | family/selector 15×14 composite와 K01 family selection은 부분 이식; product state/alpha/placement는 의도적 적응 | state 사람용 의미, family-byte runtime producer/lifetime·K01 밖 provenance, scheduler·pixel placement와 제품 mapping |
...
| K01 tile placement-level·object/frame boundary | `FUN_00464cc0` signed x/y guard·`(x-y)<<5`/`(x+y)<<4` base projection·`+0x4a0c4` fog-family indexed WORD table and low-nibble/helper relative branch, `FUN_00469330`/`FUN_00469510` second raw placement-argument subtract 및 K01 3,600 cell object/frame binding 정적 확정 | K01 전수 stream, base projection, low-nibble two/other·corner·synthetic helper branches 및 malformed/tampered rejection 재현 완료 | raw `0/16` stream을 `TileCell.elevation` base/one-raised level로 명시적 `source-backed-adaptation` 적용; source flat art `flatArtworkEmbedsRelief`와 shared ground anchor·underlay contract를 함께 소비 | `DAT_00c06e86` values/lifetime/semantics, screen/world axis·pixel anchor/pivot, helper height human meaning, 다른 map/theme 및 renderer parity |
...
| 이동·경로 탐색 | `FUN_00425b20→FUN_00445290→FUN_00444770`의 accepted-node gate, Chebyshev open-frontier capacity, strict squared-goal greedy selection/tie, ±25 window, exact eight-state candidate vector, footprint mask/OOB predicate, closest fallback·postprocess call 범위 정적 확정; workspace lifecycle/terrain semantics/product coordinate는 미확인 | bounded synthetic goal/fallback/block/OOB/cap/failure vectors 재현 완료 | `imjinrok:source-greedy-local-adapter`는 bounded kernel을 K01 metadata-selected full-path product adapter로 연결한 `source-backed-adaptation`; collision/goal adaptation/chaining/전체 accepted-node budget는 project policy이고 `core:a-star`는 기본값 | mask producer/terrain semantics, workspace reset/serialization, postprocess output/product coordinate와 full movement lifecycle |
...
| 애니메이션 | K01 class 2·3·4·7·11·12·13·16·31·82 scoped core states, class 14 상태 8/1/4와 16-ring·transient destruction, 두 영웅 상태 1·4·7·8 정적 확정; class 7·31은 source-created creation-default state 4→state-8 idle fallback, `+0x47a==0` state 8/1/7, `+0x47a!=0` state 8/1 및 resource-work state 10/11/16 frame·direction·mirror와 raw action substate 8 state-16 numeric fast-start/cadence를 포함 | 방향·phase·상태별 슬롯·flags, class 7/31 state-4 fallback/nonzero/resource-work branch와 state-16 numeric cadence, class 14 turn/effect tick 경계 및 K01 영웅 사망 완료/해제 경계 재현 | class 7·31 idle/move/walk/death와 `carriedResource.amount > 0` adapter의 carry/carry-idle, generic `gather`의 original state-10 intentional source-layout adapter, class 7 `build`/`repair`의 original state-11 intentional source-layout adapter, class 2·3·4·11·12·13·14·16·82 grid 핵심 상태 및 class 14 raw16/last-grid-facing source-backed runtime adaptation; original field를 일반화하지 않음 | class 7·31 state-4 이후 mutation/reachability·combat meaning, state 16 raw/selector 3 사람용 의미와 product mapping, resource-work 사람용 상태명·selector identity·FPS/pivot/stats/behavior, class 2/3/4·12 state 2 project policy, class 14 original accepted update→project tick/spawn mapping |
...
| 브리핑 초상화 | 정적 확정 | 재현 완료 | `hero.spr` mapping과 K01 17-entry source label table·slot bottom label 좌표는 원본 기반; 5% step의 웹 24 Hz cadence는 의도적 적응 | 원본 dispatcher wall-clock scheduler와 새 원본 변형에도 추출기·클라이언트 교차 검증 |
| `SPEECH` 대화 레이아웃 | 정적 확정 | 재현 완료 | 확정 portrait/text/label 좌표 원본 기반; 마지막 line click dismiss는 프로젝트 input adaptation | 제목·목표·버튼·글꼴을 별도 분석 |
...
| UI·입력 | 기존 범위에 더해 class 78 auto origin 1과 manual origin 0의 pending-store 우선순위·일반 공격 short-circuit, 9-slot command-grid의 successful `button.spr` `34×34` source binding·strict edge, original action 2/3/5/11/16/19/21/35/39의 CP949 source label→runtime pointer→`button.spr` pixel frame 43/6/4/16/12/45/11/10/39 및 action 61/62/63/64→pixel frame 27/26/28/29 path, `pannel.spr` `640×163` `(0,0)` locked blit과 grid 선행 order, exact-one `FUN_00421390` lock·owner flag·unlock·final-helper dispatch order, game-speed state `0..4`의 interval/3-frame groups와 `mouseinterface.spr` two-control state 정적 확정 | class 78 자동 action 40/59 record와 manual-block/replace·source-invalid·no-op 경계 부분 재현; command-grid exact rectangle/strict edge와 loader·blit gate failure, bounded action frame selection의 no-draw boundaries, selected renderer의 lock failure/success·`0x200`/`0x400` call order, game-speed interval vector와 bounded mouse-control state 재현 완료 | 원본 3×3 geometry·strict hit helper/test는 research evidence로 유지; K01 `imjinrok-k01-opening` opt-in profile은 recovered button frames를 source binding으로 사용한다. responsive 4×3 12-slot grid와 selection panel, 테마의 명시적 `EntityVisual.portrait` 또는 대표 entity frame을 쓰는 선택 이미지, five-preset local playback·localStorage setting과 web one/two-button pointer semantics는 의도적 프로젝트 UI/적응이다. 이 선택 이미지는 `portrait.spr`의 유닛·건물 의미를 주장하지 않는다. | remembered right-click reservation action/owner, 나머지 8개 magic-auto class effect, outer input/entity update 순서, computed-alias writer 부재·selected renderer subcall semantic contents·runtime surface semantic owner, source pointer routing/drag threshold와 `portrait.spr`의 유닛·건물 선택 패널 용도 |
