...
| 원본 HUD clock resource와 web clock adapter | EXE `0x004bc200` data pointer→`0x004bd2ec` `fnt\\clock.spr`, hash-bound `32×36` 20-frame SPR container는 정적 확정. frame/face/hand 의미, draw caller·placement·update/time→angle mapping은 미확인 | pointer/path·SPR header·변조 거부 vector 재현 완료; web adapter의 normalized progress/rail collision vector는 제품 테스트 | 20 source frame PNG는 source identity export만 수행하며 runtime bind 없음. simulation environment `timeOfDay01` 기반 non-interactive analog HUD clock과 responsive right-title/left-zoom-rail geometry는 의도적 적응 | loader→draw caller full flow, frame selector, original coordinates/clip/input, update cadence와 time→frame/angle mapping |
