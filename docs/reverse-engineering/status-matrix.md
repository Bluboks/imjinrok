...
| K01 봉화대 트리거 | raw-relation blocker→1,200-slot 완성 record scan→flag·K0120·native effect→post-state 반환과 native class·K01 요청 좌표 범위 정적 확정 | trigger 분기·descriptor·selector·return 및 타입/SPR·요청 좌표 9개 재현 완료 | K01 adapter 9개 모두 exact static identity/source; class 13 핵심 상태와 class 14 상태 8/1/4 grid animation 이식, class 82는 base-frame still | 행동·stats·class 14 opaque/death·class 82 animation·owner·원본 생성/최종 배치 정책 뒤 isolated opt-in 확대 |
...
| 애니메이션 | 조선 창병 상태 1·2 이동, K01 class 13·class 14 상태 8/1/4와 두 영웅 상태 1·4·7·8, 공격 효과와 영웅 사망 phase/update-unit 수명 정적 확정 | 방향·phase·상태별 슬롯·flags, class 14 special/opaque consumer와 K01 영웅 사망 완료/해제 경계 재현 | 조선 창병 일반 이동, class 13·class 14 grid 핵심 상태와 두 영웅 핵심 상태 이식; 원본 영웅 사망 수명 미이식 | 원본 update→FPS/24 Hz 변환, class 14 opaque/death, class 82, 조선 창병 상태 2 통합과 특수 분기 base 연결 |
