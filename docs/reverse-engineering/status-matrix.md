...
| K01 공통 임무 목표 모달 결합 | 공통 모달 자체, `0x3f0` handler 반환 생산 경로와 K01 인덱스 1→K0110 목표 텍스트·K01 map·handler 결합 정적 확정 | 기하·입력·정상 로드 cleanup과 별도로 상태 반환·signed WORD 인덱스·K01 레코드 결합 재현 완료; 로더 실패·sound/latch는 미재현 | 기하·입력·cleanup 독립 모듈만 원본 기반, 결합은 분석 전용이며 장면 연결 없음 | application state `0x16`의 완전한 사용자 진입 조건과 컨트롤 `0x005527b0` 자원·사각형 복원 |
...
- K01 UI 파일럿 후보로 선택한 공통 임무 목표 모달은 `FUN_00449090`의 상태 `0x3f0/0x3f1`,
  ...
  `정적 확정`·`재현 완료`지만 분석 전용이다. 글꼴·줄바꿈과 application state `0x16`의 완전한
  사용자 진입 조건은 미확정이라 장면 연결을 보류했다. 실행 중 목표 추적 HUD는 별도 프로젝트 전용
  구현이다.
