...
| 공통 임무 목표 모달 K01 UI 파일럿 후보 | 공통 모달 자체는 정적 확정; `0x3f0`의 상위 return-value/간접 생산 경로와 K01 텍스트 레코드 결합은 미확정 | 좌표·종료 판정·소비·정규화·draw 순서와 정상 로드 자원 cleanup의 clear-lock 성공·실패만 재현 완료; 로더 실패·sound/latch는 미재현 | 해당 재현 범위의 자원·계산·입력·cleanup 모듈은 원본 기반, 장면 연결 없음 | `0x3f0`을 만드는 return-value/간접 경로와 `DAT_0088afcc` 레코드의 K01 결합 복원 |
...
- K01 UI 파일럿 후보로 선택한 공통 임무 목표 모달은 `FUN_00449090`의 상태 `0x3f0/0x3f1`,
  ...
  직접 참조 집합에는 literal `0x3f0` 쓰기가 없지만 handler 반환값을 기록하는 참조가 있으므로,
  상위 return-value/간접 생산 경로와 텍스트 레코드의 K01 선택은 미확정이다. 따라서 K01 사용은
  주장하지 않고 문자열·글꼴과 함께 장면 연결을 보류했다. 실행 중 목표 추적 HUD는 별도 프로젝트
  전용 구현이다.
