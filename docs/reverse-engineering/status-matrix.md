...
| 원본 fog renderer 경계 | `FUN_00467de0`의 low-nibble `0` early skip와 center state `0`→state `4`→`8`, nonzero non-8→state `8`, center `8`→`FUN_00469510` base path dispatch, state 4/8 별도 8-neighbor mask→`0x004bf9c4` lookup, caller isometric projection과 six-argument push order, `FUN_0046a530`의 `map+0x4a0c4+x*180+y` family byte+`0x3c`→fog record 60..74와 black 75, corrected `FUN_0046d650` lookup arithmetic와 raw shift, normal fog header `32×16`/96/`1024×48`의 signed-IDIV frame algebra·state별 3×2/six-subframe call path, state-8 `FUN_00452b30`의 palette channel→32×32/64×64 LUT→packed 5:6:5→`object+0x19a0` remap을 정적 확정. lifecycle evidence가 literal `0`=visible, `4`=explored, `8`=unseen을 보완하며, caller/callee 한정 local `64×48` composite anchor는 `(32,0)`이다 | EXE·SPR·map hash/header, dispatch raw range/anchors·lookup 16-vector, center `0`/`4`/`8`·low-nibble·mask omission vectors, corrected projection·six-argument K01 vector, family stream·record address·frame vector·loop bound와 lifecycle sparse vector, state-8 LUT 32×32·64×64 exhaustive minimum replay 재현 완료 | family/selector 15×14 composite와 K01 family selection은 부분 이식하며, lifecycle source-backed literal semantics에 따라 adapter가 product center `0`을 state `4`→`8` ordered boundary path, center `4`를 state `8` boundary path, center `8`을 base path로 연결하도록 보정 중이다. base tile coverage는 유지하고 explored silhouette·unseen gradient opacity는 명시적 product alpha adaptation으로 소비한다. corrected raw shift와 canonical coverage diagnostic을 소비하며, native palette remap과 browser alpha 동등성·web scheduling·broader placement는 주장하지 않는다 | product visibility lifecycle/grid conversion, family-byte runtime producer/lifetime·K01 밖 provenance와 arbitrary-map low-nibble zero gate, state-8 final palette-index table/runtime palette lifecycle, viewport ownership, renderer-wide pivot·clip·mode, alpha/blend와 web scheduler |
...
| K01 map-data extraction protocol v1 | MAP/EXE hash·header·declared stride/extent와 field_0x32514/object/frame/fog/placement/raw shift/passability streams 및 corrected helper arithmetic 정적 확정 범위 | 결정론 JSON/TS·tamper/truncate/dimension/coordinate 실패 벡터와 corrected stream/digest 재현 완료 | source-raster native fact는 원본 기반 경계로 유지; map-level canonical coverage는 `의도적 적응` | human terrain/elevation/fog semantics, auxiliary renderer paths와 원작 전체 compositor parity |
| K01 tile placement-level·object/frame boundary | `FUN_00464cc0` signed x/y guard·`(x-y)<<5`/`(x+y)<<4` base projection·`+0x4a0c4` fog-family indexed WORD table and low-nibble/helper relative branch, `FUN_00469330`/`FUN_00469510` second raw placement-argument subtract, `FUN_00466f20` K01 `width*64 × (height*32+200)` raster·y-outer/x-inner order·screen point/caller argument and 3,600 cell object/frame binding 정적 확정. `FUN_0046d650` lookup은 `map+0x51f54+selector*0x7e90+x*180+y`로 corrected 되었고, `FUN_00462b80`은 8-byte stride의 15 indexed WORD를 초기화해 family 0=`0`, family 1..14=`9`와 adjacent zero WORD를 쓴다 | corrected K01 helper/raw-shift replay와 corner/synthetic vectors, placement/projection/compositor fixture digest와 malformed/tampered rejection 재현 완료 | corrected raw shift는 source placement metadata로만 유지한다. K01 authored/product physical surface는 neutral(3,600개 `TileCell.elevation=0`)이며 원본 physical elevation을 복원한 주장이 아니다. 제품의 canonical coverage는 `의도적 적응`이고, selected source frame은 top-edge `(32,0)` anchor와 opaque black clear 뒤 global y→x raster replay를 소비한다. corrected outputY/raw-shift arithmetic는 [placement boundary](mechanics/k01-tile-placement-elevation-boundary.md)를 따른다 | table alias/computed writer, lifetime/order와 사람용 height/elevation 의미; `FUN_00481c50` row overwrite의 wider consumer/lifetime; default gameplay compositor 밖의 auxiliary/nondefault paths, broader pivot/axis와 full renderer parity |
...
| 원본 엔티티 전수 시각 프로필 | 타입 `+0x0c` signed-WORD render offset, current-SPR dimension pivot split, 2단계 building selector/destination table, 건설 본체/overlay frame range와 class-41 conditional draw를 정적 확정 | 95/95 SPR header·hash·dimension/frame, table byte hash, class 50/57 vectors와 conditional quarantine fixture 재현 완료 | [`originalEntityTypeProfiles.generated.ts`](../../packages/shared/src/originalEntityTypeProfiles.generated.ts)의 분석 소비 표면; runtime current frame/elevation/clip은 별도 계산 | wall-clock/FPS calibration, shared effect secondary producer의 product mapping, runtime entity field/elevation/palette/clip |
| 원본 building far-cell placement와 product geometry adapter | `FUN_00438930`/`FUN_00438aa0`의 signed center/footprint fields, far occupied-cell formula, separate SPR slot pixel dimensions `+0x1da/+0x1dc`, pivot branches, cached projection consumers와 draw-field writes를 정적 확정. `+0x1e3/+0x1e4`는 SPR pixel dimensions가 아님 | EXE·function raw range·call-edge·opcode anchor·representative SPR hash와 3x3/3x2/2x2/1x1·signed offset·pivot/cached projection vectors를 bounded replay 완료 | source profile의 10개 building class logical extent/source-center anchor를 shared resolver가 placement·collision·range·build work·client geometry에 전달하고, product building-only adapter가 actual interaction tiles와 elevation-aware far contact/polygon·semantic center·local sprite offset·bounds hit/depth를 소비 | native table/cell-cache producer·lifetime·camera transform, full source movement ownership, raw owner semantics, construction/compositor parity |
| K01 coverage clarification | 원본 native compositor에는 별도 coverage layer가 없고 index-0/검정 clear와 선택 YTL row span만 관찰된다 | 제품은 map/profile 수준 canonical source-art coverage를 `의도적 적응`으로 선택하며, legacy `source-raster-underlay`는 authored per-cell underlay로 보존한다 | native fact와 product coverage를 분리해 기록·검증 | native coverage layer와 product coverage를 혼동하지 않기 |
| 엔티티 자료구조 | source handle lifecycle 범위 정적 확정; 전체 구조체는 추정 | allocator/generation/validity/release 범위와 v3/v4→v5 legacy footprint/result migration 재현 완료 | T01 source runtime v5가 bounded `entityRuntime`/`occupancy` SSOT와 opening/class-52 admission adapter를 부분 이식하고 semantic-unit removal hook이 기존 release를 소비함; construction-time stale-owner bridge는 현재 semantic blocker와 unknown owner를 보존하며 proven stale owner만 복사본에서 제거함. v3 class-52 `1×1` save는 기존 owner history를 보존하면서 active 입력 빈 cell만 static-confirmed `3×3`으로 확장하고 result namespace를 주입함 | 전체 mega-struct·모든 runtime writer/alias·identity mapping·원본 death timing 교차 확인; legacy raw movement/overwrite history의 완전한 replay |
| 게임 틱과 메인 루프 | 기존 scheduler 증거에 더해 K01 stage 1 `state 1→stage 1 map→state 3→FUN_00447bc0`와 `FUN_004464c0→FUN_004481d0→FUN_0048ddb0→timer→FUN_0048a5c0→accepted FUN_00447360`의 accepted source-update 순서·direct edge·entity/projectile pass count를 정적 확정. cold-start `config.hq` open failure는 settings object `+0x14` selector DWORD를 2로 초기화하고 successful 0x1d4-byte transfer는 해당 offset을 유지할 수 있음도 정적 확정. mode `1`의 조건부 base `50 ms`와 guard zero/nonzero writer는 확인했지만 K01 stage-1 concrete mode value는 미확정 | raw selector·feedback·wrap·거부, cold-start load-failure/default·load-success/retention, mode/guard, same/distinct raw tick, result-before-pool, entity/projectile count vectors 재현 완료; [branch precedence·early transition·tracked raw cache/counter/next-state replay correction verified](mechanics/k01-accepted-update-scheduler.md) | 분석 전용 accepted-update trace와 fixture, 독립 포트와 project loop는 미연결 | K01 mode·guard concrete producer/alias lifetime, persisted config validity/lifetime, raw clock units, exact wall-clock frequency와 project scheduling 정책 |
...
| K01 봉화대 트리거 | raw-relation blocker→1,200-slot 완성 record scan→flag·K0120·native effect→post-state 반환과 descriptor slot/OOB/exact create·native 1×1 reinforcement occupancy overwrite 정적 확정 | trigger 분기·descriptor·selector·return, slot/경계/overlap/WORD-wrap 및 타입/SPR·요청 좌표 9개 재현 완료; source-vs-product K0120 차분 vector(정적 hash gate 포함), objective projection·migration·remove/rebuild/save vectors와 독립 blocked-wait sequence가 통과 | source runtime v5의 completed-beacon/native admission과 K01 beacon/result/scenario policy가 source trigger·native effect를 project objective/result boundary에 연결; K01 adapter 9개 exact static identity/source와 in-bounds exact request-position create를 소비 | class 12 state-2·class 14 generic Facing/runtime tick, 네 class stats/behavior, raw owner, native scheduler·movement/death/full result presentation lifecycle |
| K01 occupancy-owner transition | native allocate→record init→action-1 clear/helper/writer, mobile owner `0x00ac2da4` slot overwrite, mask `0x00ae27e4`, movement `0x32` coordinate commit, outer release/death retain·stale-clear 경계를 정적 확정 | 20 vector replay, function/anchor/call-edge hash, root-independent path와 EXE/artifact/fixture tamper rejection 재현 완료 | source runtime v5는 generation 없는 slot-WORD owner cell과 엄격한 admission collision/OOB boundary를 production SSOT로 사용하고 semantic-unit removal은 기존 release를 호출; v3 legacy beacon save는 owner history 보존형 3×3 projection을 거침; source movement overwrite/scheduler/death timing은 연결하지 않음 | `0x00ad2ac4` alternate alias, raw owner/player 의미, K01 밖 footprint, computed/alias writer와 scheduler serialization, legacy raw movement/overwrite replay |
| K01 opening footprint anchor | type writer argument 8/9→type `+0x14/+0x16`, creator copy→runtime `+0x1e3/+0x1e4`, map loader source-index order→wrapper generation/slot→action-1 writer, centered `floor(extent/2)` anchor, mask-OR→slot-owner store와 per-cell OOB skip, class 48/49/50/51/57/58/60/62/63의 15 opening records와 dynamic class 52의 3×3 initializer 정적 확정 | odd/even/mixed extents, class 49 `(5,4)` 3×3, class 7 `(7,6)` 1×1 control, class 52 vector, edge/OOB, overwrite, active-gate·pre-copy와 artifact tamper 9-vector 재현 완료 | source runtime opening seed와 completed-beacon adapter가 source-confirmed footprint를 소비하며, shared resolver가 source-center extent를 placement·collision·range·build work·client geometry에 전달; v3/v4 save는 owner history 보존형 v5 compatibility projection을 사용함; sprite/pivot/native cell-cache/full movement는 별도 경계 | raw owner/player 의미, alternate/non-mobile writer, construction/damaged/overlay/timing/pivot/stats/commands/behavior, scheduler serialization, legacy raw movement/overwrite replay |
...
| 이동·경로 탐색 | `FUN_00425b20→FUN_00445290→FUN_00444770`의 accepted-node gate, Chebyshev open-frontier capacity, strict squared-goal greedy selection/tie, ±25 window, exact eight-state candidate vector, footprint mask/OOB predicate, closest fallback·postprocess call 범위와 class 2 type `+0x3c/+0x50`→entity `+0x4ee/+0x4ea` 초기화, `+0x4ee→+0x4f2` raw accumulator read/write 및 signed threshold 뒤 recovered current-coordinate pair commit [bridge](mechanics/k01-class2-locomotion-bridge.md) 정적 확정; workspace lifecycle/terrain semantics/product coordinate와 raw field의 속도·변위 단위는 미확인 | bounded synthetic goal/fallback/block/OOB/cap/failure 및 class 2 raw field/cadence·accumulator/coordinate commit boundary vectors 재현 완료; 두 product pathfinder의 mobile-obstruction 40-tick 대기·save/load 후 재개 vector 통과 | `imjinrok:source-greedy-local-adapter`는 bounded kernel을 K01 metadata-selected full-path product adapter로 연결한 `source-backed-adaptation`; collision/goal adaptation/chaining/전체 accepted-node budget는 project policy이고 `core:a-star`는 기본값. mobile blocker 대기 중 movement intent를 idle animation으로 분리하고 blocker 제거 후 재탐색한다. 제품 `movementSpeed`와 24 Hz displacement는 project fallback으로 유지 | mask producer/terrain semantics, workspace reset/serialization, postprocess output/product coordinate, other alias/computed writer, full movement lifecycle와 original update→24 Hz conversion |
| K01 source coordinate bridge | map cell과 exact placement의 signed-WORD·x-major·bounds·1x1/exact pair 및 class-2 raw/current/accumulator·Ryu start/end/route signed-WORD 범위를 domain별로 분리하고, opening/dynamic building의 source footprint anchor/width/height는 [opening footprint anchor](mechanics/k01-opening-footprint-anchor.md)에서 닫음. C01 accepted scheduler→entity/projectile 순서와 E01 slot/generation/coordinate linkage는 확인 범위만 연결; locomotion/projectile scale·project anchor·24 Hz/FPS·identity는 미확정 | source-word/map-edge/placement nine-pair/accumulator threshold·signed boundary/projectile accepted route 및 opening-footprint 9-vector·artifact tamper 재현 완료 | bounded source footprint resolver와 client/simulation consumers가 production에 통합됨; full coordinate bridge, locomotion/projectile source unit·producer/consumer·GridPoint scale·scheduler clock은 미연결 | full source movement/locomotion, projectile source unit mapping, project coordinate adapter, scheduler clock and project identity mapping |
...
- K01 updater는 general raw presence, 봉화대 direct return, class 76, class 78 순으로 처리한다.
  ...
  raw result clock은 millisecond source로 확인되어 bounded v5 policy에 연결했다. native scheduler cadence와 full result/identity/presentation mapping은 미확정이다.
...
- 클래스 1~95 타입 정체·자원 카탈로그와 signed render offset/current-SPR dimension 기반 pivot은
  `95/95 static-confirmed`·generated profile로 닫았고, 원본 class에 gameplay binding된 visual은
  이 프로필을 소비한다.
- 35개 building 공통 프로필의 construction phase·healthy/damaged body frame을 재현했다. 정적으로
  닫힌 continuous overlay 12개는 profile에 매핑했으며, conditional/random draw 2개는 quarantine해
  제품 상태에 연결하지 않았다.
- `SPEECH` 초상화 ID 계산식은 `hero.spr` 기준으로 `정적 확정`·이식 완료
- 위 범위를 벗어난 행동·방향·틱/FPS·건물 특수 상태와 compositor 순서는 별도 미확정이며,
  제품은 확인된 source frame만 선택적으로 이식한다.
