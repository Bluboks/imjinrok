...
| 애니메이션 | 조선 창병 상태 1·2 이동과 K01 권율·유성룡 상태 1·4·7·8의 슬롯·프레임·방향 정적 확정 | 해당 방향·phase·상태별 슬롯과 flags 분기 재현 | 조선 창병 일반 이동, 두 영웅 idle·일반 이동·공격·사망 이식 | 재생 속도·타격 phase·사망 표시 수명, 조선 창병 상태 2 통합과 특수 분기 base 연결 |
...
- 권율은 클래스 76 `generalk11/12/13.spr`, 유성룡은 클래스 78 `generalk31/32.spr`로
  ...
  방향은 정적 확정·이식했다. 재생 속도·타격 phase·피격 반응·사망 표시 수명은 미확정이다.
...
- 클래스 76 `조선 권율`과 78 `조선 유성룡`의 전용 SPR와 상태 8 idle·1 일반 이동·4 공격·
  7 사망 이식 완료
- 영웅 재생 속도·타격 phase·피격·사망 표시 수명, 조선 창병 상태 2 프로젝트 통합과 상태 1
  특수 분기의 클래스 2 base는 다음 단계
