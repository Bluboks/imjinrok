...
| K0110 briefing outer update cadence | `PeekMessageA` present/absent, state `0x14`→`0x0047f300`→`0x004824c0`, readiness→one consumer, direct caller projection 정적 확정 | strict `>`/DWORD-wrap/negative WORD와 one-record/retained-boundary vectors 재현 완료 | 분석 전용, production 변경 없음 | exact wall-clock/fps, sprite compositor, alternate caller K0110 reachability |
