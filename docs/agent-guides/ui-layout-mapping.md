...
`DAT_00552998 = 0x3f0`을 만드는 상위 handler return-value/간접 생산 경로와
`DAT_0088afcc` 기반 텍스트 레코드가 K01에 결합되는 경로는 미확정이다. 따라서 위 사실은 공통
모달 자체의 기준선이며 K01 사용 증거가 아니다.

두 텍스트 블록의 X·최대 폭·세로 중심은 확정했지만 입력 레코드 의미, 문자열, 글꼴과 줄바꿈 규칙은
아직 확정하지 않았다. 원본 SPR 로더 실패와 컨트롤 sound/latch 부수효과는 정적-only이고 재현·
이식하지 않았다. 자원·계산·입력 판정과 정상 로드 자원 cleanup 모듈만 원본 기반이며 실제 장면
연결은 보류했다. 현재 실행 중인 목표 추적 패널은 이 원본 모달과 다른 프로젝트 전용 구현이다.
