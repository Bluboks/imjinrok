...
| K01 공통 임무 목표 모달 결합 | 원본 frame 자원·좌표·닫기 판정·종료 cleanup 모듈 추가, 장면 연결 없음 | `0x3f0` handler 반환 생산 경로와 K01 인덱스 1→K0110 목표 텍스트·K01 map·handler 결합 정적 확정 | 기하·입력·정상 로드 cleanup과 별도로 상태 반환·signed WORD 인덱스·K01 레코드 결합 재현 완료; 로더 실패·sound/latch 미재현 | 분석·재현 범위는 원본 기반, 글꼴·줄바꿈·정확한 사용자 표시 트리거 미확정이라 화면 연결 보류 |
...
4. K01 HUD, 선택 패널, 목표·진행 표시와 미션 대화 전체 레이아웃을 정적으로 복원 — 공통 임무
   ...
   복원했지만 application state `0x16`의 완전한 사용자 진입 조건, 컨트롤 자원·사각형, 글꼴과
   실행 중 목표 추적 HUD·루트 좌표계·선택 패널은 남음
