...
| 공통 임무 목표 모달 K01 UI 파일럿 후보 | 원본 frame 자원·좌표·닫기 판정·종료 cleanup 모듈 추가, 장면 연결 없음 | 공통 모달 흐름은 정적 확정; `0x3f0`의 상위 return-value/간접 생산 경로와 K01 텍스트 결합은 미확정 | 좌표·종료 판정·draw 순서·정상 로드 자원 cleanup만 원본 벡터와 클라이언트 교차 테스트; 로더 실패·sound/latch 미재현 | 교차 테스트 범위만 원본 기반, K01 사용·문자열·글꼴 미확정이라 화면 연결 보류 |
...
4. K01 HUD, 선택 패널, 목표·진행 표시와 미션 대화 전체 레이아웃을 정적으로 복원 — 파일럿으로
   선택한 공통 임무 목표 모달 자체의 frame·내용·닫기·종료 경로는 복원했지만 `0x3f0`의 상위
   return-value/간접 생산 경로와 K01 텍스트 결합, 글꼴과 실행 중 목표 추적 HUD·루트 좌표계·
   선택 패널은 남음
