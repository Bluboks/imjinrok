...
| K01 공통 임무 목표 모달 결합 | 원본 frame·좌표·닫기·cleanup 모듈과 semantic `open-objective-modal` UI-domain 계약 추가, 장면 연결 없음 | 진입·목표 컨트롤에 더해 ordered pending overwrite와 `FUN_00449090`의 소비·활성·reset·실패 경로 정적 확정 | request→strict release→`0x3f0`과 dispatcher 목표 생명주기 재현 완료; loader 내부 객체 부수효과·오디오 자원 미재현 | UI-domain 계약까지 원본 기반; mechanism 발행 source, gameplay-panel 정체, 목표 label·글꼴·줄바꿈 미확정이라 화면 연결 보류 |
...
4. K01 HUD, 선택 패널, 목표·진행 표시와 미션 대화 전체 레이아웃을 정적으로 복원 — 공통 임무
   ...
   목표 모달 소비·reset까지 연결했다. semantic action의 mechanism 발행 source, gameplay-panel의
   화면상 정체, 목표 label·글꼴·줄바꿈, 실행 중 목표 추적 HUD·루트 좌표계·선택 패널은 남음
