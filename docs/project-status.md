...
정적 복원·재현했고 독립 계산을 부분 이식했다. 원본 caller의 전체 signed-WORD 좌표 범위, 원본
틱의 초 단위·프로젝트 좌표 변환과 실제 투사체 연결, 현재 시뮬레이션의 사망 엔티티 제거 시점은
아직 원본 기반이 아니다.
...
| 전투 | 프로토타입, 유성룡 좌표 accepted subset `0..32767` 독립 계산 부분 이식 | K01 권율·유성룡 공격 효과 phase·회복, 권율 직접 피해와 유성룡 subtype `0x0c` 수명·WORD 피해·raw 적용 gate 확정 | 두 영웅 phase·회복, 권율 직접 피해와 유성룡 투사체 정상·wrap·gate·실패 벡터 재현 | K01 영웅 제한 범위 부분 재현·부분 이식, 실제 전투 연결은 원본 caller 전체 좌표 범위와 좌표·시간 변환 대기 |
