...
| K01 공통 임무 목표 모달 결합 | 원본 frame·좌표·닫기·cleanup 모듈과 semantic action 계약, K01 HUD objective button→UI event queue→private active request의 staged 연결; 사용자 표시 presenter 없음 | 진입·목표 컨트롤에 더해 ordered pending overwrite와 `FUN_00449090`의 소비·활성·reset·실패 경로 정적 확정 | request→strict release→`0x3f0`과 dispatcher 목표 생명주기 재현 완료; 프로젝트 event 중복·재생성 경계 테스트; loader 내부 객체 부수효과·오디오 자원 미재현 | `original-parity`는 원본 action 의미만 한정; HUD button trigger·event bridge·active request는 프로젝트 전용; gameplay-panel 정체, presenter, 목표 label·글꼴·줄바꿈 미확정 |
...
4. K01 HUD, 선택 패널, 목표·진행 표시와 미션 대화 전체 레이아웃을 정적으로 복원 — 공통 임무
   ...
   trigger로 삼아 semantic action을 `UIScene` private active request까지 한 번 전달하는 staged
   연결만 두었다. 사용자에게 보이는 presenter,
   gameplay-panel의 화면상 정체, 목표 label·글꼴·줄바꿈, HUD 루트 좌표계·선택 패널은 남음
