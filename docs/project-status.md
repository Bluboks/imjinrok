...
`japanese-turtle-tank`의 상태 8/1/4 grid frame·mirror도 정적 확정·이식했다.
class 82 visual만 base frame 0 still을 사용한다. generic open-point 배치가 최종 위치를
옮기거나 생략할 수 있다.
...
| K01 캠페인 | 처음부터 결과까지 프로토타입, native 증원 9개 exact static identity/source adapter | 표준 entry timer reset, 봉화대→K0120, native class/요청 좌표, class 13·class 14 scoped 핵심 animation, latch→timer→commit, result presentation→final route 범위 확정 | 요청 좌표·exact static identity/source 9/9, class 13 및 class 14 grid frame/direction, native effect·timer·presentation·route 경계 재현 | 단기 팬 리마스터 MVP, class 14 opaque/death·class 82 animation과 증원 stats/behavior·최종 배치·raw clock/result policy 미완료 |
...
1. K01 native 증원 class 82 animation 및 class 14 opaque/death와 네 class 생성·점유 정책을 독립 복원
