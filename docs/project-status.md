...
| K01 공통 임무 목표 모달 결합 | 원본 frame 자원·좌표·닫기 판정·종료 cleanup 모듈 추가, 장면 연결 없음 | 기존 결합에 더해 state `0x16`의 complete structured direct-reference 생산, Escape·gameplay-panel request, K01 `buttons201.spr` 목표 컨트롤 사각형·입력 정적 확정 | request→state `0x16`→mode 1→strict release→`0x3f0` 연결 재현 완료; SPR 런타임 로더 실패·오디오 자원 미재현 | 분석 전용; gameplay-panel 정체, 목표 label·글꼴·줄바꿈과 shared 상태 계약 미확정이라 화면 연결 보류 |
...
4. K01 HUD, 선택 패널, 목표·진행 표시와 미션 대화 전체 레이아웃을 정적으로 복원 — 공통 임무
   ...
   복원했고 state `0x16`의 direct 생산과 목표 컨트롤 입력까지 연결했다. gameplay-panel의 화면상
   정체, 목표 label·글꼴·줄바꿈, 실행 중 목표 추적 HUD·루트 좌표계·선택 패널은 남음
